"""Document chunking strategies for PDF documents.

This module provides various chunking strategies for PDF documents,
optimized for maintaining context and improving retrieval accuracy.
"""
...
class DocumentChunker:
    ...
    def chunk_by_tokens(
        ...
        for i in range(0, len(tokens), self.max_tokens - self.overlap_tokens):
            ...
            chunk = self._create_chunk(chunk_text, metadata, chunk_id, start_char, end_char)
            chunks.append(chunk)
            chunk_id += 1

        return chunks

    def chunk_by_at_commands(
        self, markdown_content: str, metadata: dict[str, Any]
    ) -> list[DocumentChunk]:
    ...
    def _create_chunk(
        self, content: str, metadata: dict[str, Any], chunk_id: int, start_char: int, end_char: int
    ) -> DocumentChunk:
    ...
    def _split_chunk_at_boundary(self, lines: list[str]) -> list[str] | None:
